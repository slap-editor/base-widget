...
BaseWidget.prototype.resolveBinding = function (key, source1, source2, etc) {
  return BaseWidget.resolveBinding.apply(null,
    [key, BaseWidget.prototype.getBindings.call(this)].concat([].slice.call(arguments, 1))
  );
};
BaseWidget.resolveBinding = function (key, source1, source2, etc) {
  var bindings = [].slice.call(arguments, 1).reduce(function (bindings, sourceBindings) {
    return bindings.merge(sourceBindings);
  }, _()).toObject();
...
BaseWidget.prototype._initHandlers = function () {
  ...
  self.on('element keypress', function (el, ch, key) {
    switch (BaseWidget.prototype.resolveBinding.call(self, key)) {
